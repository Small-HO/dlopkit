 ...
 */
package com.small.camerakit.internal.ui.widget;
...
public class CheckView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCountable) {
            if (mCheckedNum != UNCHECKED) {
                initBackgroundPaint();
                canvas.drawCircle((float) SIZE * mDensity / 2, (float) SIZE * mDensity / 2, BG_RADIUS * mDensity, mBackgroundPaint);
                initTextPaint();
    ...
    private void initShadowPaint() {
        if (mShadowPaint == null) {
            ...
            float stop3 = 1.0f;
            mShadowPaint.setShader(new RadialGradient((float) SIZE * mDensity / 2, (float) SIZE * mDensity / 2, gradientRadius * mDensity, new int[]{Color.parseColor("#00000000"), Color.parseColor("#0D000000"), Color.parseColor("#0D000000"), Color.parseColor("#00000000")}, new float[]{stop0, stop1, stop2, stop3}, Shader.TileMode.CLAMP));
        }
    }
